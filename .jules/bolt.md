...
## 2026-11-05 - [Pre-fill Template Elements Outside Render Loops]
**Learning:** Assigning static or shared state (like the current unit display `kg`/`lb`) to DOM elements inside hot render loops (like table generation) introduces redundant JavaScript execution overhead. Because the same unit is applied to every row, this breaks the zero-allocation architectural goal for highly bound operations.
**Action:** Pre-fill shared or static state directly on the `<template>.content` element itself when the state changes (e.g. updating the unit globally). By doing this, `cloneNode(true)` natively duplicates the correct content directly in C++, completely avoiding per-row JavaScript assignments in hot loops.
