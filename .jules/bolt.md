...
## 2024-05-19 - [Conditional DOM Writes for Layout Thrashing]
**Learning:** Unconditionally writing to DOM properties like `textContent` or `innerHTML` inside hot render loops, even when the new value is identical to the current one, forces the browser to cross the JavaScript-C++ boundary. This triggers unnecessary layout invalidations and negatively impacts rendering performance.
**Action:** In hot render paths, always wrap DOM writes in conditional checks (e.g., `if (node.textContent !== currentUnit) node.textContent = currentUnit;`). This allows the CPU's branch predictor to efficiently bypass the expensive DOM operation when the text is already correct, adhering to zero-allocation and mechanical sympathy principles.
