...
## 2024-05-19 - Conditional DOM Writes in Hot Paths
**Learning:** In hot render paths dealing with recycled DOM nodes (like cloned `<template>` nodes in a table), unconditional DOM writes (e.g., `node.textContent = value`) force the JavaScript engine to cross the JS-C++ boundary repeatedly, even if the value hasn't changed.
