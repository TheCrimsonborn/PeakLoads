...
## 2024-05-18 - [Conditional DOM Writes]
**Learning:** Checking for equality (`el.textContent !== target`) before mutating `textContent` in the DOM significantly prevents V8 DOM barrier crossing overhead and layout thrashing. Because `textContent` returns strings, explicit casting via `String(calculatedNumber)` is necessary before the strict equality check to ensure type safety.
**Action:** Always check `if (el.textContent !== value)` prior to updating DOM text to prevent redundant DOM updates when elements are being reused or conditionally written to.
