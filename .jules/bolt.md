...
## 2026-11-01 - [Pre-fill HTML Templates using Static Structural Classes]
**Learning:** When attempting to zero-allocate render loops by pre-filling shared state (like unit strings) inside `<template>` elements instead of cloned nodes, it is critical that the target elements within the template have their structural attributes (e.g., `class="unit-display"`) statically defined in HTML. If the class is applied conditionally in the original JS loop and missing from the template HTML, queries like `template.content.querySelectorAll(".unit-display")` will fail to find the node, causing bugs where the pre-filled state is lost.
**Action:** Always move static structural attributes (like invariant classes) into the HTML `<template>` markup when migrating to a pre-fill zero-allocation pattern. Never rely on JS to construct the fundamental state of the base template.
