...
## 2024-05-20 - [Conditional DOM Writes]
**Learning:** Unconditionally assigning properties like `textContent` in hot render loops causes expensive boundary crossings from JS into C++ engine logic even when the actual string value has not changed, resulting in measurable layout overhead.
**Action:** In hot render paths dealing with recycled DOM nodes, strictly use conditional checks before DOM writes (e.g., `if (node.textContent !== currentUnit) node.textContent = currentUnit;`). Do not perform unconditional DOM writes; instead, rely on the CPU's branch predictor to skip the expensive DOM boundary crossing when the text already matches.
