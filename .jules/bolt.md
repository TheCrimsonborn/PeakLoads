...
## 2024-07-28 - Conditional DOM Writes in Render Paths
**Learning:** In hot render paths dealing with recycled DOM nodes or frequent state updates, assigning text content to a node unconditionally forces the browser to cross the JS/C++ boundary, creating unnecessary mutation events and layout invalidation flags, even if the text value hasn't changed.
**Action:** Strictly use conditional checks before DOM writes (e.g., `if (node.textContent !== currentUnit) node.textContent = currentUnit;`). Rely on the CPU's branch predictor to skip the expensive DOM boundary crossing when the text already matches.
