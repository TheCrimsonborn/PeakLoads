...
## 2026-11-13 - [Conditional DOM textContent Writes]
**Learning:** Unconditionally assigning to `el.textContent` crosses the JS-C++ boundary and invalidates the DOM even if the assigned string is strictly equal to the existing content, causing unnecessary layout thrashing and V8 overhead.
**Action:** Always wrap `.textContent` assignments in a strict inequality check (`if (el.textContent !== String(newVal)) { el.textContent = newVal; }`) to rely on the CPU's branch predictor to skip the expensive DOM boundary crossing when the text already matches, perfectly aligning with mechanical sympathy and Zero-Allocation optimizations.
