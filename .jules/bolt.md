...
## 2024-11-13 - [Conditional DOM writes for textContent]
**Learning:** In hot render paths dealing with DOM nodes, V8 executes property assignment directly to C++. However, when setting properties like `.textContent`, you should still verify the content isn't the same to allow CPU's branch predictor to skip the expensive DOM boundary crossings when the text already matches.
**Action:** Always use conditional checks before DOM writes (e.g., `if (node.textContent !== targetStr) node.textContent = targetStr;`). Explicitly cast calculated numbers to strings (e.g., `String(val)`) before comparing them against DOM properties like `.textContent` to prevent redundant writes due to implicit casting rules.
