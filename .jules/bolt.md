...
## 2026-09-05 - [De-duplicate DOM element iteration arrays]
**Learning:** Initializing multiple `NodeList` iterators into arrays dynamically over time, or executing sequential similar DOM update loops, adds slight initialization delays and iteration overhead in the UI update pipeline, particularly against zero-allocation standards.
**Action:** Pre-calculate expected capacities (`NodeList.length`) to pre-allocate unified state tracking arrays (e.g., `new Array(total)`). This condenses multiple identical node mutation actions into a singular index-based loop.
