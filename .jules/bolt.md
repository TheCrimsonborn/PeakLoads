...
## 2026-11-20 - [Conditional DOM Writes for textContent]
**Learning:** Unconditional assignments to `node.textContent` in hot paths (like event listeners or localization loaders) force the browser to execute DOM mutation logic and recalculate layouts, even if the text value has not actually changed.
**Action:** Always wrap `textContent` assignments in a conditional check (e.g., `if (el.textContent !== newVal) el.textContent = newVal;`). Because `textContent` always returns a string, ensure any calculated numbers are explicitly cast to `String` (e.g., `String(val)`) before comparison to guarantee the strict equality check (`!==`) functions correctly.
