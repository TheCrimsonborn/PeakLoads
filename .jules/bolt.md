...
## 2026-11-12 - [Avoid Unconditional textContent Updates in Render Loops]
**Learning:** Unconditionally assigning to `node.textContent` inside hot render loops or UI update functions (even if the value is the same) forces the V8 engine to cross the JS-to-C++ boundary repeatedly. The browser engine must often process these as new mutations, potentially invalidating styles and layout, causing rendering bottlenecks and layout thrashing, despite there being no visual change.
**Action:** Always cast the new value to a primitive (e.g., `String()`) and perform a strict equality check (`if (node.textContent !== newValue) node.textContent = newValue;`) before setting `textContent`. This relies on the CPU's branch predictor to skip the heavy JS-to-C++ DOM assignment boundary if the text is identical, drastically improving execution speed in zero-allocation architectures.
