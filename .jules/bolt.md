...
## 2026-11-12 - [Combine NodeLists for Duplicate Updates]
**Learning:** Having multiple similar NodeLists or collections that we iterate over separately (like `staticUnitDisplays` and `templateUnitDisplays`) leads to code duplication and inefficient duplicate loops on hot paths.
**Action:** Combine such collections into a single pre-allocated array during initialization to allow a single, de-duplicated index-based loop. Use conditional checks (`!==`) before DOM writes to skip expensive JS-C++ boundary crossings when values already match.
