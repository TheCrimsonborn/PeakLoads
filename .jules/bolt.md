...
## 2024-07-26 - [Remove string allocations in hot render loop]
**Learning:** Concatenating strings or dynamically generating symbols like `%` during a hot DOM render loop (e.g., inside `populateRow` iterating over hundreds of items) generates unnecessary memory garbage and causes garbage collection pauses, which degrades UI responsiveness.
**Action:** Move static symbols (like `%`) directly into the HTML `<template>` definition. Pre-fill any other shared static text (like current units) on the `template.content` nodes immediately upon initialization or state change, *before* the hot loop begins. During the loop, only assign primitive numeric data (`row.percent`, `row.weight`) directly to the pre-allocated `.textContent` of the specific dynamically changing elements, relying on the browser's native C++ `cloneNode(true)` to handle all static text.

## 2024-07-26 - [Suppress Block Duplication in SonarCloud]
**Learning:** SonarCloud's "Duplication on New Code" Quality Gate failure cannot be fixed by inline `// NOSONAR` comments when the duplication spans multiple lines (e.g., two identical loops iterating over different NodeLists).
**Action:** Instead of suppressing, refactor the code to eliminate the duplication entirely. If you have two loops doing the same thing to different sets of nodes (like elements in the main document vs. elements inside `<template>`s), combine those nodes into a single, pre-allocated static Array during initialization. This allows the hot path to use one single loop, satisfying both zero-allocation and CI duplication rules.
