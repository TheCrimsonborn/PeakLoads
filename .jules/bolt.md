...
## 2024-07-27 - [Add Conditional DOM Updates]
**Learning:** Checking if a DOM element's `textContent` matches the new value before updating it prevents the browser from triggering unnecessary parsing, layout trashing, and repaint cycles. DOM properties like `textContent` always return a string, so calculated numbers must be cast to strings before comparison to ensure accurate strict equality checks (`!==`).
**Action:** When updating DOM elements, especially in hot loops or render functions, implement conditional checks before writing. Ensure types match (e.g., cast numbers to strings) when comparing against DOM properties.
