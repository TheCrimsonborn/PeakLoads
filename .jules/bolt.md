...
## 2026-07-25 - [Pre-fill Template Content]
**Learning:** Performing per-row DOM manipulation (like setting classes or unit text content) after cloning templates inside a hot loop is redundant and wastes CPU cycles. Since `cloneNode(true)` natively copies the DOM structure efficiently in C++, any static layout properties or universally shared state (like active weight unit) can be modified directly on the base `<template>` content *before* the hot loop starts.
**Action:** Always extract static configurations or universally shared state (like a unit label) and apply them to the master `<template>.content` node directly, rather than applying them dynamically to every cloned row during rendering iteration.
