...
## 2026-11-10 - [Conditional Write for Template Fallbacks]
**Learning:** When optimizing hot render loops by pre-filling HTML `<template>` elements, completely removing dynamic property assignments (like `textContent` or `className`) assuming `cloneNode` handles it breaks functionality for conditional rendering (e.g. toggling between empty strings vs unit values in fallback blocks).
**Action:** Retain necessary dynamic assignments inside conditional blocks, but wrap them in strict branch-predictor conditional checks (e.g., `if (node.textContent !== value) node.textContent = value;`) to achieve the performance benefit without breaking behavior.
