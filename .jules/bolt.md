...
## 2024-11-13 - [Cache string values to avoid redundant DOM writes]
**Learning:** Assigning a string to \`textContent\` on multiple DOM nodes repeatedly, even if the value hasn't changed, causes the browser to cross the JS-C++ boundary repeatedly, which wastes CPU cycles and can induce layout thrashing on cold initialization paths.
**Action:** When updating elements in a static NodeList, use a module-level variable to cache the last assigned string. Perform a strict equality early return (\`if (_lastValue === newValue) return;\`) to completely avoid the \`textContent\` loop if the value hasn't changed.
